...
class Animasu extends Provider_1.default {
    ...
    async getAnimes(params) {
        try {
            const res = await axios_1.default.get(params?.search ? `${this.getBaseUrl()}/page/${params.page || 1}/` : `${this.getBaseUrl()}/pencarian/`, {
                params: {
                    s: params?.search || "",
    ...
    async getAnime(slug) {
        try {
            const res = await axios_1.default.get(`${this.getBaseUrl()}/anime/${slug}/`);
            const $ = cheerio.load(res.data);
            const infox = $(".infox");
            // Parsing data utama
    ...
    async getStreams(episodeSlug) {
        try {
            const streams = [];
            const res = await axios_1.default.get(`${this.getBaseUrl()}/${episodeSlug}/`);
            const $ = cheerio.load(res.data);
